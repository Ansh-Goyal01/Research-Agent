...
def safe_write(path, content, mode="w"):
    ...
    if not _is_allowed(path):
        raise PermissionError(f"Write to {path} is not allowed. Must be inside outputs/ or state/ or logs/")
...
def save_section(section_name, content):
    path = config.OUTPUTS_SECTIONS / f"{section_name}.md"
    return safe_write(path, content)

def save_final(filename, content):
    final_path = config.OUTPUTS_FINAL / filename
    if final_path.exists():
        archive_name = f"{filename}.{datetime.now().strftime('%Y%m%d_%H%M%S')}.bak"
